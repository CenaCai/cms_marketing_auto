> ⚠️ **LEGACY / 已归档**：此 Next.js 运营台已不再是 `cms_marketing_auto` 仓库的主交付物。整体项目已转向基于 Mautic 开源框架定制（白标 + 简体中文），详见仓库根目录 `README.md` 与 `mautic-csts-kit/`。本目录仅作历史参考与能力对照，不再随主流程维护。
...
后台默认以 **mock 模式**运行（不真实发信、只记录日志），因此**无需任何外部服务即可把整条链路跑通**。当基础流程确认无误后，再在「设置」里填入你的邮箱（SMTP）切换为真实发送。

最小可跑通路径（4 步）：

1. **模板（生产文案 + 图片）** → `/templates`
   - 新建模板：填名称、邮件主题、HTML 正文（支持 `{{first_name}}` 等变量）。
   - 点「＋ 插入图片」把图片 URL 加到正文；右侧实时预览渲染效果。
2. **客户名单（邮箱地址）** → `/contacts`
   - 新增客户，至少填邮箱（或电话）；这是邮件的收件人。
3. **活动（发送邮件）** → `/campaigns`
   - 新建活动并绑定上一步的模板。
   - 点「发送」→ 勾选收件人（可多选 = 批量发送）→ 确认。
   - 发送后即时显示「发送记录」（success / failed + 错误原因）。
4. **配置真实邮箱** → `/settings` → 「邮件发送通道」
   - 选择 `smtp`，填写 SMTP 主机 / 端口 / 账号 / 密码 / 发件人，保存即生效（无需重启）。
   - 填错或未填会给出明确提示（如「SMTP 未配置：请在后台设置填写 SMTP_HOST…」）。

> 端到端已验证：模板(含图片+变量) → 客户(邮箱) → 活动(绑模板) → 发送 → 发送记录 success。
