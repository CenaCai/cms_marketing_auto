...
class TagController extends FormController
{
    ...
    /**
     * Bulk import tags from an uploaded .txt file (comma separated) or pasted text.
     * Tags are created without description (description is edited individually later).
     *
    ...
    public function importAction(Request $request)
    {
        ...
        $action    = $this->generateUrl('mautic_tagmanager_import');

        $indexRedirect = fn (array $flashes = []) => $this->postActionRedirect([
            'returnUrl'       => $returnUrl,
        ...
        if (Request::METHOD_POST === $request->getMethod()) {
            ...
            if ($request->request->has('tag_import_cancel')) {
                return $indexRedirect();
            }

            $raw  = '';
            $file = $request->files->get('tag_import_file');
            if ($file instanceof UploadedFile && $file->isValid()) {
                $raw = (string) file_get_contents($file->getPathname());
            } else {
                $raw = (string) $request->request->get('tag_import_text', '');
            }

            // Strip UTF-8 BOM if present.
            $raw = preg_replace('/^\xEF\xBB\xBF/', '', (string) $raw);
            // Uploaded files and browser textareas submit UTF-8, but be defensive against
            // non-UTF-8 byte sequences (e.g. GBK) so they never reach the DB and trigger a
            // "1366 Incorrect string value" charset error.
            if (!mb_check_encoding($raw, 'UTF-8')) {
                $converted = @mb_convert_encoding($raw, 'UTF-8', 'UTF-8, GB18030, GBK, BIG-5');
                if (false !== $converted && '' !== $converted) {
                    $raw = $converted;
                }
            }
            // Normalise line breaks and full-width comma into a plain comma separator.
            $raw = str_replace(["\r\n", "\r", "\n", '，'], ',', $raw);
            ...
            foreach ($parts as $name) {
                ...
                if (isset($seen[$key])) {
                    continue; // duplicate within the same upload
                }
                $seen[$key] = true;

                if ($model->getRepository()->countOccurrences($name) > 0) {
                    ++$duplicate;
            ...
            if (0 === $created && 0 === $duplicate) {
                return $indexRedirect([
                    [
                        'type' => 'error',
                        'msg'  => 'mautic.tagmanager.tag.import.error.empty',
                    ],
                ]);
            }
            ...
            return $indexRedirect([
                [
                    'type'    => 'notice',
                    'msg'     => 'mautic.tagmanager.tag.import.success',
                    'msgVars' => [
                        '%created%'   => $created,
                        '%duplicate%' => $duplicate,
                    ],
        ...
        return $this->delegateView([
            ...
            'contentTemplate' => '@MauticTagManager/Tag/import.html.twig',
            'passthroughVars' => [
                'activeLink'    => '#mautic_tagmanager_index',
                'route'         => $action,
                'mauticContent' => 'tagmanager',
